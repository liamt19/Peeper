...
ifneq ($(OS),Windows_NT)
	...
	ifeq ($(UNAME_S),Darwin)
		OUT_DIR =
		FIX_OUTPUT = mv ./bin/Release/osx-arm64/publish/Peeper ./Peeper
	endif
...
#  -c Release                  Builds using the Release configuration in Peeper.csproj
...
release:
	dotnet publish $(PROJ) $(BUILD_OPTS)
	$(FIX_OUTPUT)

#  This will/might only succeed if you have the right toolchain
aot:
	-dotnet publish $(PROJ) $(BUILD_OPTS) $(AOT_OPTS)

512: $(BINDINGS_FILE)
	dotnet publish $(PROJ) $(BUILD_OPTS) -p:DefineConstants="AVX512"
	$(FIX_OUTPUT)

aot_512:
	-dotnet publish $(PROJ) $(BUILD_OPTS) $(AOT_OPTS) -p:DefineConstants="AVX512"
